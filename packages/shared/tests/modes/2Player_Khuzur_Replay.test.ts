...
describe('2-Player Khuzur E2E Game Replay', () => {
  ...
  const injectCards = (
    playerId: string,
    cardsInfo: { suit: string; rank: number; isJoker?: boolean }[],
  ) => {
  ...
  it('successfully simulates the full 12-turn logged Khuzur game', () => {
    ...
    removeCard('p1', 12, 'spades');
    removeCard('p1', 12, 'hearts');
    removeCard('p1', 8, 'clubs');
    ...
    removeCard('p2', 14, 'spades');
    removeCard('p2', 12, 'diamonds');
    removeCard('p2', 12, 'clubs');
    ...
    let atksT3 = [
      findCard('p2', 11, 'clubs')!,
      findCard('p2', 11, 'hearts')!,
      findCard('p2', 8, 'spades')!,
    ];
    ...
    injectCards('p2', [
      ...
      { suit: 'hearts', rank: 9 },
      { suit: 'clubs', rank: 14 },
    ]); // fixed 8c to 9h to make math valid
    let atksT5 = [
      findCard('p1', 8, 'spades')!,
      findCard('p1', 8, 'hearts')!,
      findCard('p1', 7, 'clubs')!,
    ];
    let defsT5 = [
      findCard('p2', 9, 'spades')!,
      findCard('p2', 9, 'hearts')!,
      findCard('p2', 14, 'clubs')!,
    ];
    ...
    let atksT6 = [
      findCard('p2', 9, 'diamonds')!,
      findCard('p2', 9, 'clubs')!,
      findCard('p2', 9, 'hearts')!,
    ];
    let defsT6 = [
      findCard('p1', 14, 'diamonds')!,
      findCard('p1', 11, 'clubs')!,
      findCard('p1', 11, 'hearts')!,
    ];
    ...
    let atksT7 = [
      findCard('p1', 3, 'spades')!,
      findCard('p1', 3, 'hearts')!,
      findCard('p1', 7, 'hearts')!,
    ];
    ...
    expect(
      DurakEngine.canDefendMass(
        [findCard('p2', 3, 'diamonds')!],
        [findCard('p1', 2, 'clubs')!],
        state.huzurSuit,
      ),
    ).toBe(true);
    ...
    ]); // Kd, 4s, 2d (trumps over 7h)
    let atksT9 = [
      findCard('p2', 3, 'hearts')!,
      findCard('p2', 3, 'spades')!,
      findCard('p2', 7, 'hearts')!,
    ];
    let defsT9 = [
      findCard('p1', 13, 'diamonds')!,
      findCard('p1', 4, 'spades')!,
      findCard('p1', 2, 'diamonds')!,
    ];
    ...
    let atksT10 = [
      findCard('p1', 13, 'clubs')!,
      findCard('p1', 13, 'hearts')!,
      findCard('p1', 10, 'clubs')!,
    ];
    ...
    const remainingPlayers = Array.from(state.players.entries()).filter(
      ([_, p]) => p.hand.length > 0,
    );
