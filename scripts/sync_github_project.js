...
const inProgressPatterns = ['Card Animations'];
...
for (const item of data.items) {
  ...
  try {
    execSync(
      `gh project item-edit --id ${item.id} --project-id PVT_kwHOCsmXI84BU8hD --field-id ${FIELD_ID} --single-select-option-id ${targetStatusOpt}`,
      { stdio: 'inherit' },
    );
  } catch (err) {
