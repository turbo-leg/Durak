...
try {
  ...
  for (const localIssue of localIssues) {
    ...
    if (!targetStatusId) {
      console.warn(
        `Unknown state '${localIssue.state}' for local issue #${localIssue.number}. Skipping.`,
      );
      continue;
    }

    // Try to find the matching item in the GitHub Project board by title or number
    let matchingItem = projectItems.find(
      (item) =>
    ...
    if (!itemId) {
      ...
      try {
        ...
        const addJson = execSync(
          `gh project item-add ${PROJECT_NUM} --owner "${OWNER}" --url "${issueUrl}" --format json`,
          { encoding: 'utf-8' },
        );
    ...
    if (itemId) {
      ...
      console.log(
        `Could not find or add a matching GitHub project item for local issue: "${localIssue.title}"`,
      );
    }
  }
